...
class Portofolio extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->akses = $this->content_model->getAkses($level, 'portofolio');
    }
    ...
    public function data()
    {
        ...
        if (in_array($cek, $this->akses)) {
            ...
            $list = array(
                'status'    => false,
                'message'   => "Anda tidak berhak untuk mengakases Portofolio ini",
                'data'      => array()
            );
        }
        ...
        if(in_array($cek,$this->akses)){
            ...
            if (empty($row)) {
                ...
                if ($this->form_validation->run()) {
                    ...
                    header('location:' . base_url() . "admin/Portofolio");
                } else {
                    $array = array(
                        'status'    => TRUE,
                        'error'     => TRUE,
                    ...
                    //print_r($array);
                    header('location:' . base_url() . "admin/Portofolio/form");
                    //header('Content-Type: application/json');
                ...
                if ($this->form_validation->run()) {
                    ...
                    header('location:' . base_url() . "admin/Portofolio/form/" . $content_id);
                }
            }
        }
    }
        ...
        if (in_array($cek, $this->akses)) {
            ...
            if ($_FILES['userfile']['name'] != "") {
                if (!$this->upload->do_upload()) {
                    ...
                    header('location:' . base_url() . "admin/Portofolio/form/" .$tipe);
                } else {
                    //SImpan
                    $file = $this->upload->data("file_name");
                    $data = array(
                    ...
                    if (empty($row)) {
                        ...
                        if ($this->form_validation->run()) {
                            ...
                            $error[] = $this->_file_resize(_DIR_MEDIA_ . "/" . $file, _DIR_MEDIA_THUMB_ . $file, 500, 500);
                            $icon[] = $this->_file_resize(_DIR_MEDIA_ . "/" . $file, _DIR_MEDIA_ICON_ . $file, 50, 50);
                            header('location:' . base_url() . "admin/Portofolio/". $tipe);
                        } else {
                            $array = array(
                                'status'    => TRUE,
                                'error'     => TRUE,
                ...
                if (empty($row)) {
                    ...
                    if ($this->form_validation->run()) {
                        ...
                        //echo json_encode($array);
                        //$this->session->set_flashdata('error', $array);
                        header('location:' . base_url() . "admin/Portofolio/form/". $tipe ."/" . $content_id);
                    }
                }
            }
        } else {
            //header('Content-Type: application/json');
            //echo json_encode(array("status" => False,'error'=>TRUE, "message"=> "Anda tidak berhak untuk mengakases Portofolio ini"));
            $this->session->set_flashdata('error', array("Anda tidak berhak untuk mengakases Portofolio ini"));
            header('location:' . base_url() . "login");
        }
    ...
    function delete($id)
    {
        ...
        if (in_array($cek, $this->akses)) {
            ...
            header('Content-Type: application/json');
            echo json_encode(array("status" => FALSE, "message" => "Anda tidak berhak untuk mengakases Portofolio ini"));
        }
    }
    function excel()
    {
