...
class Prodi extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->akses = $this->content_model->getAkses($level, 'prodi');
    }
        ...
        if(in_array($cek,$this->akses)){
            ...
            if (empty($row)) {
                ...
                if ($this->form_validation->run()) {
                    ...
                    $this->session->set_flashdata('error', $array);
                    //print_r($array);
                    header('location:' . base_url() . "admin/Prodi/form");
                    //header('Content-Type: application/json');
                    //echo json_encode($array);
                }
            } else {
        ...
        if (in_array($cek, $this->akses)) {
            ...
            if ($_FILES['userfile']['name'] != "") {
                if (!$this->upload->do_upload()) {
                    ...
                    header('location:' . base_url() . "admin/Prodi/form/" .$tipe);
                } else {
                    //SImpan
                    $file = $this->upload->data("file_name");
                    $data = array(
                    ...
                    if (empty($row)) {
                        ...
                        if ($this->form_validation->run()) {
                            ...
                            $error[] = $this->_file_resize(_DIR_MEDIA_ . "/" . $file, _DIR_MEDIA_THUMB_ . $file, 500, 500);
                            $icon[] = $this->_file_resize(_DIR_MEDIA_ . "/" . $file, _DIR_MEDIA_ICON_ . $file, 50, 50);
                            header('location:' . base_url() . "admin/Prodi/". $tipe);
                        } else {
                            $array = array(
                                'status'    => TRUE,
                                'error'     => TRUE,
                ...
                if (empty($row)) {
                    ...
                    if ($this->form_validation->run()) {
                        ...
                        //echo json_encode($array);
                        //$this->session->set_flashdata('error', $array);
                        header('location:' . base_url() . "admin/Prodi/form/". $tipe ."/" . $content_id);
                    }
                }
            }
        } else {
            //header('Content-Type: application/json');
            //echo json_encode(array("status" => False,'error'=>TRUE, "message"=> "Anda tidak berhak untuk mengakases Prodi ini"));
            $this->session->set_flashdata('error', array("Anda tidak berhak untuk mengakases Prodi ini"));
            header('location:' . base_url() . "login");
        }
    ...
    function delete($id)
    {
        ...
        if (in_array($cek, $this->akses)) {
            ...
            header('Content-Type: application/json');
            echo json_encode(array("status" => FALSE, "message" => "Anda tidak berhak untuk mengakases Prodi ini"));
        }
    }
    ...
    public function fileUpload()
    {
        if (!empty($_FILES['userfile']['name'])) {
            $file = "POST_" . date('dmY') . "_" . str_replace(' ', '_', $_FILES['userfile']['name']);
            $this->_file_upload(_DIR_MEDIA_, $file, 'gif|jpg|png|pdf');
            if (!$this->upload->do_upload()) {
                $error = $this->upload->display_errors();
                //echo $error;exit;
                $this->session->set_flashdata('error', $error);
