...
class Pengumuman extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->akses = $this->content_model->getAkses($level, 'pengumuman');
    }
    ...
    function lampiran($id = "")
    {
        ...
        $tipe = "Pengumuman";
        if (empty($id)) $modul = "Content </li><li> Tambah </li><li>" . ucwords($tipe) . "</li><li>Lampiran</li>";
        else $modul = "Content </li><li>" . ucwords($tipe) . "</li><li>Lampiran</li>";
        if (in_array($save, $this->akses)) {
            $data = array(
                'list_m_kategori' => $this->content_model->getM_kategori(),
                'menu'  => array(),
                'content_id' => $id,
                'post'  => $this->content_model->getcontent_by_id($id),
                'modul' => $modul,
                'tipe'  => $tipe,
                'id' => $id,
                'akses' => $this->akses
            );
            $content = $this->load->view('admin/content/view_lampiran', $data, true);
            $view = array(
                'content'   => $content
            );
            $this->load->view('template/admin/view_layout', $view);
    ...
    function save_lampiran()
    {
        ...
        if (in_array($cek, $this->akses)) {
            ...
            $lampiran_id = $this->input->post('id_lampiran');
            $content_id = $this->input->post('content_id');
            $link = str_replace(' ', '-', strtolower($this->input->post('judul_lampiran')));
            $link = str_replace('&', 'dan', $link);
            $link = str_replace('"', '', $link);
            $row = $this->content_model->getlampiran_by_id($lampiran_id);
            $data = array(
                'content_id' => $this->input->post('content_id'),
                'tgl_lampiran' => date('Y-m-d H:i:s'),
                'judul_lampiran' => trim($this->input->post('judul_lampiran')),
                'isi_lampiran' => trim($this->input->post('isi_lampiran')),
                'jenis_lampiran' => trim($this->input->post('jenis_lampiran')),
                'link_lampiran' => $link,
                'userinput' => $this->session->userdata("username"),
            );
            if (empty($row)) {
                $this->form_validation->set_rules('judul_lampiran', 'Judul Lampiran', 'required');
                $this->form_validation->set_rules('isi_lampiran', 'Isi Lampiran', 'required');
                if ($this->form_validation->run()) {
                    $insert = $this->content_model->insertlampiran($data);
                    $array = array(
                        'status'    => TRUE,
                        'error'     => FALSE,
                    ...
                    //header('location:' . base_url() . "admin/prodi/lampiran/" .$content_id);
                } else {
                    $array = array(
                        'status'    => TRUE,
                        'error'     => TRUE,
                        ...
                        'err_judul_lampiran' => form_error('judul_lampiran'),
                        'err_isi_lampiran' => form_error('isi_lampiran'),
                    );
                ...
                $this->form_validation->set_rules('isi_lampiran', 'Isi Lampiran', 'required');
                if ($this->form_validation->run()) {
                    $this->content_model->updatelampiran($data, $lampiran_id);
                    //header('location:' . base_url() . "admin/Prodi");
                    $array = array(
                        'status'    => TRUE,
                        'error'     => FALSE,
                        'csrf'      => $this->security->get_csrf_hash(),
                        'message'   => "Lampran Berhasil Di Update"
                    );
                    header('Content-Type: application/json');
                    echo json_encode($array);
                } else {
                    ...
                    header('Content-Type: application/json');
                    echo json_encode($array);
                    //header('location:' . base_url() . "admin/Prodi/form/" . $content_id);
                }
            }
        }
    }
    function editlampiran($id = "")
    {
        $cek = array('aksi' => 'Edit');
        if (in_array($cek, $this->akses)) {
            $row = $this->content_model->getlampiran_by_id($id);
            if (!empty($row)) {
                $response = array(
                    'status'    => true,
                    'message'   => "OK",
            ...
            $response = array(
                'status'    => false,
                'message'   => "Anda tidak berhak untuk mengedit Lampiran"
            );
        }
        header('Content-Type: application/json');
    ...
    public function data()
    {
        ...
        if (in_array($cek, $this->akses)) {
            ...
            $list = array(
                'status'    => false,
                'message'   => "Anda tidak berhak untuk mengakases Pengumuman ini",
                'data'      => array()
            );
        }
        header('Content-Type: application/json');
    ...
    function data_lampiran($prodiid)
    {
        $cek = array('aksi' => "Index");
        if (in_array($cek, $this->akses)) {
            ...
            $list = array(
                'status'    => true,
                'message'   => "OK",
                'data'     => $this->content_model->getLampiran($prodiid),
            );
        } else {
            $list = array(
                'status'    => false,
                'message'   => "Anda tidak berhak untuk mengakases Prodi ini",
                'data'      => array()
            );
        }
        header('Content-Type: application/json');
        ...
        if(in_array($cek,$this->akses)){
            ...
            if (empty($row)) {
                ...
                if ($this->form_validation->run()) {
                    ...
                    $this->session->set_flashdata('error', $array);
                    //print_r($array);
                    header('location:' . base_url() . "admin/Pengumuman/form");
                    //header('Content-Type: application/json');
                    //echo json_encode($array);
                }
            } else {
                ...
                if ($this->form_validation->run()) {
                    ...
                    header('location:' . base_url() . "admin/Pengumuman/form/" . $content_id);
                }
            }
        }
    }
        ...
        if (in_array($cek, $this->akses)) {
            ...
            if ($_FILES['userfile']['name'] != "") {
                if (!$this->upload->do_upload()) {
                    ...
                    header('location:' . base_url() . "admin/Pengumuman/form/" .$tipe);
                } else {
                    //SImpan
                    $file = $this->upload->data("file_name");
                    $data = array(
                    ...
                    if (empty($row)) {
                        ...
                        if ($this->form_validation->run()) {
                            ...
                            $error[] = $this->_file_resize(_DIR_MEDIA_ . "/" . $file, _DIR_MEDIA_THUMB_ . $file, 500, 500);
                            $icon[] = $this->_file_resize(_DIR_MEDIA_ . "/" . $file, _DIR_MEDIA_ICON_ . $file, 50, 50);
                            header('location:' . base_url() . "admin/Pengumuman/". $tipe);
                        } else {
                            $array = array(
                                'status'    => TRUE,
                                'error'     => TRUE,
                ...
                if (empty($row)) {
                    ...
                    if ($this->form_validation->run()) {
                        ...
                        //echo json_encode($array);
                        //$this->session->set_flashdata('error', $array);
                        header('location:' . base_url() . "admin/Pengumuman/form/". $tipe ."/" . $content_id);
                    }
                }
            }
        } else {
            //header('Content-Type: application/json');
            //echo json_encode(array("status" => False,'error'=>TRUE, "message"=> "Anda tidak berhak untuk mengakases Pengumuman ini"));
            $this->session->set_flashdata('error', array("Anda tidak berhak untuk mengakases Pengumuman ini"));
            header('location:' . base_url() . "login");
        }
    ...
    function delete($id)
    {
        ...
        if (in_array($cek, $this->akses)) {
            ...
            header('Content-Type: application/json');
            echo json_encode(array("status" => FALSE, "message" => "Anda tidak berhak untuk mengakases Pengumuman ini"));
        }
    }
    function deletelampiran($id)
    {
        $cek = array('aksi' => 'Delete');
        if (in_array($cek, $this->akses)) {
            $this->content_model->deletelampiran($id);
            header('Content-Type: application/json');
            echo json_encode(array("status" => TRUE, "message" => "Data Berhasil dihapus"));
        } else {
            header('Content-Type: application/json');
            echo json_encode(array("status" => FALSE, "message" => "Anda tidak berhak untuk Menghapus Lampiran"));
        }
    }
    function excel()
    {
        ...
        if (!empty($_FILES['userfile']['name'])) {
            ...
            if (!$this->upload->do_upload()) {
                ...
                $data=array(
                    ...
                    'namafile' => $filename,
                    'keterangan'    => $_FILES['userfprile']['name'],
                    'status_media'  => 1
                );
