...
function getMekanisme(start){
    ...
    var url=base_url + "mekanisme/data?q=" + search + "&start=" +start;
    $.ajax({
        url     : url,
        type    : "GET",
        dataType: "json",
        ...
        success : function(data){
            ...
            tabel+="<td>"+mekanisme[i]["mekanisme_pelayanan"]+"</td>";
                    tabel+='<td class=\'text-right\'><button type=\'button\' class=\'btn btn-success btn-xs\' onclick=\'edit("' +mekanisme[i]["id_mekanisme"] +'")\'><span class=\'fa fa-pencil\' ></span></button>|';
                    tabel+='<button type=\'button\' class=\'btn btn-danger btn-xs\' onclick=\'hapus("' +mekanisme[i]["id_mekanisme"] +'")\'><span class=\'fa fa-remove\' ></span></td>';
                    tabel+="</tr>";
                }
                $('#data').html(tabel);
                //Create Pagination
                if(data["row_count"]<=limit){
                    ...
                    var btnFirst="<button class='btn btn-default btn-sm' onclick='getMekanisme(0)'><span class='fa fa-angle-double-left'></span></button>";
                    if(curIdx>1){
                        var prevSt=((curIdx-1)*data["limit"])-jmlData;
                        btnFirst+="<button class='btn btn-default btn-sm' onclick='getMekanisme("+prevSt+")'><span class='fa fa-angle-left'></span></button>";
                    }

                    var btnLast="";
                    if(curIdx<jmlPage){
                        var nextSt=((curIdx+1)*data["limit"])-jmlData;
                        btnLast+="<button class='btn btn-default btn-sm' onclick='getMekanisme("+nextSt+")'><span class='fa fa-angle-right'></span></button>";
                    }
                    btnLast+="<button class='btn btn-default btn-sm' onclick='getMekanisme("+lastSt+")'><span class='fa fa-angle-double-right'></span></button>";
                    ...
                    if(jmlPage>=25){
                        ...
                        for (var j = idx_start; j<=idx_end; j++) {
                            ...
                            btnIdx+="<button class='btn " +btn +" btn-sm' onclick='getMekanisme("+ st +")'>" + j +"</button>";
                        }
                    }else{
...
function add(){
    ...
    $('.modal-title').text('Tambah Data Mekanisme'); 
}
function save(){
    var url;
    url = base_url + "mekanisme/save";
...
function edit(id)
{
    ...
    $('#form')[0].reset(); 
    $.ajax({
        url : base_url + "mekanisme/edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
...
function hapus(id){
    ...
    function(isConfirm){
      if (isConfirm) {
        $.ajax({
            url : base_url + "mekanisme/delete/" +id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
