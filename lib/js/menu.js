            ...
            if(data["status"]==true){
                ...
                for(var i=0; i<jmlData;i++){
                    ...
                    if (menu[i]["menu_top"] == 1) {
                        ...
                        tabel += '<td>';
                        tabel += '<label class="switch switch-small">';
                        tabel += '<input type="checkbox" id="menu_top' + menu[i]["menu_id"] + '"  value="1" onclick="newTop(\'' + menu[i]["menu_id"] + '\')" />';
                        tabel += '<span></span>';
                        tabel += '</label>';
                        tabel += '</td>'
                    ...
                    if(menu[i]["menu_newtab"]==1){
                        ...
                        tabel+='<td>';
                        tabel+='<label class="switch switch-small">';
                        tabel += '<input type="checkbox" id="menu_newtab' + menu[i]["menu_id"] +'" value="1" onclick="newTab(\''+menu[i]["menu_id"]+'\')" />';
                        tabel+='<span></span>';
                        tabel+='</label>';
                        tabel+='</td>'
                        //tabel+="<td style='width:100px;'><button class='btn btn-danger btn-sm'>Non Aktif</button></td>";
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if(data["status"]==false){
                ...
                if (menu.menu_top == 1) $('#menu_top').prop("checked", true);
...
function newTab(menu_id){
    if ($('#menu_newtab'+menu_id).is(':checked')) {
...
function newTop(menu_id) {
    var status = $('#menu_top' + menu_id).is(':checked');
    //alert(status);
    if ($('#menu_top' + menu_id).is(':checked')) {
