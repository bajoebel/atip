        ...
        success : function(data){
            ...
            if(data["status"]==true){
                ...
                for(var i=0; i<jmlData;i++){
                    ...
                    tabel += "<td>" + form[i]["form_lampiran"] + "</td>";

                    if (form[i]["form_status"] == 1) {
                        tabel += '<td style="width:100px;">';
                        tabel += '<label class="switch switch-small">';
                        tabel += '<input type="checkbox" checked value="1" id="status' + form[i]['form_id'] + '" onclick="setStatus(\'' + form[i]["form_id"] + '\')"/>';
                        tabel += '<span></span>';
                        tabel += '</label>';
                        tabel += '</td>'
                        ...
                        tabel += '<td style="width:100px;">';
                        tabel += '<label class="switch switch-small">';
                        tabel += '<input type="checkbox" value="1" id="status' + form[i]['form_id'] + '" onclick="setStatus(\'' + form[i]["form_id"] + '\')" />';
                        tabel += '<span></span>';
                        tabel += '</label>';
                        tabel += '</td>'
...
function setLampiran(id) {
    if ($('#lampiran' + id).is(':checked')) {
        var url = base_url + "form/setlampiran/" + id + "/1";
    } else {
        var url = base_url + "form/setlampiran/" + id + "/0";
    }
    console.log(url);
    $.ajax({
        url: url,
...
function setStatus(id) {
    if ($('#status' + id).is(':checked')) {
        var url = base_url + "form/setstatus/" + id + "/1";
    } else {
        var url = base_url + "form/setstatus/" + id + "/0";
    }
    console.log(url);
    $.ajax({
        url: url,
...
function downloadExcel(formid){
    window.open(base_url+"form/excel/"+formid);
}
function cetak(formid) {
    window.open(base_url + "form/cetak/" + formid);
}