...
function getRole(start){
    ...
    var url=base_url + "role/data?q=" + search + "&start=" +start+ "&limit=" +limit;
    $.ajax({
        url     : url,
        type    : "GET",
        dataType: "json",
        ...
        success : function(data){
            ...
            if(data["status"]==true){
                ...
                for(var i=0; i<jmlData;i++){
                    ...
                    tabel+='<button type=\'button\' class=\'btn btn-default btn-xs\' onclick=\'hakakses("' +role[i]["role_id"] +'")\'><span class=\'glyphicon glyphicon-key\' ></span> Hak Akses User</button>';
                    tabel+='<button type=\'button\' class=\'btn btn-success btn-xs\' onclick=\'edit("' +role[i]["role_id"] +'")\'><span class=\'glyphicon glyphicon-pencil\' ></span> Edit</button>';
                    tabel+='<button type=\'button\' class=\'btn btn-danger btn-xs\' onclick=\'hapus("' +role[i]["role_id"] +'")\'><span class=\'glyphicon glyphicon-remove\' ></span> Hapus</div></td>';
                    tabel+="</tr>";
                }
                $('#data').html(tabel);
                //Create Pagination
                if(data["row_count"]<=limit){
                    ...
                    var btnFirst="<button class='btn btn-default btn-sm' onclick='getRole(0)'><span class='glyphicon glyphicon-angle-double-left'></span></button>";
                    if(curIdx>1){
                        var prevSt=((curIdx-1)*data["limit"])-data["limit"];
                        btnFirst+="<button class='btn btn-default btn-sm' onclick='getRole("+prevSt+")'><span class='glyphicon glyphicon-angle-left'></span></button>";
                    }

                    var btnLast="";
                    if(curIdx<jmlPage){
                        var nextSt=((curIdx+1)*data["limit"])-data["limit"];
                        btnLast+="<button class='btn btn-default btn-sm' onclick='getRole("+nextSt+")'><span class='glyphicon glyphicon-angle-right'></span></button>";
                    }
                    btnLast+="<button class='btn btn-default btn-sm' onclick='getRole("+lastSt+")'><span class='glyphicon glyphicon-angle-double-right'></span></button>";
