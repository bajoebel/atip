...
var v = new Vue({
    ...
    methods:{
         showAll(){ axios.get(this.url+"user/showAll?limit="+this.rowCountPage+"&page="+this.currentPage).then(function(response){
          console.log(response);
                 if(response.data.users == null){
                     v.noResult()
                    }else{
                        v.getData(response.data.users,response.data.record_count, this.currentPage);
                    }
            })
        },
          searchUser(){
      ...
      var formData = new FormData();
        ...
        getData(users, record_count, halaman){
            v.emptyResult = false; // become false if has a record
            v.totalUsers = record_count //get total of user
            v.currentPage=halaman;
            //v.totalUsers = 100;
            //alert(v.currentPage);
